...
using iMES.Entity.DomainModels.Sys;
...
namespace iMES.Builder.Services
{
    public partial class Sys_TableInfoService
    {
        ...
        /// <returns></returns>
        /// <returns></returns>
        private string GetMysqlTableSchema(string dbConnection)
        {
            try
            {
                string dbName = dbConnection.Split("Database=")[1].Split(";")[0]?.Trim();
        ...
        /// </summary>
        /// <returns></returns>
        private string GetMySqlModelInfo(string dbService)
        {
            return $@"SELECT
DISTINCT
        ...
        CASE
            ...
            WHERE
                   table_name = ?tableName {GetMysqlTableSchema(GetConnection(dbService))};";
        }
	...
	SELECT CASE WHEN t.ColumnType IN ('DECIMAL','smallmoney','money') THEN 
        ...
        /// <returns></returns>
        private ISqlDapper GetDapper(string dbServer)
        {
            ...
            switch (dbServer)
            {
                case "ServiceDbContext":
                    sqlDapper = DBServerProvider.SqlDapperService;
                    break;
                case "ReportDbContext":
                    sqlDapper = DBServerProvider.SqlDapperReport;
                    break;
                default:
                    sqlDapper = DBServerProvider.SqlDapper;
                    break;
            }
            return sqlDapper;
        }
        ...
        public string CreateEntityModel(Sys_TableInfo sysTableInfo)
        {
            ...
            {
                case "MySql":
                    sql = GetMySqlModelInfo(sysTableInfo.DBServer);
                    break;
                case "PgSql":
            ...
            List<TableColumnInfo> tableColumnInfoList = GetDapper(sysTableInfo.DBServer).QueryList<TableColumnInfo>(sql, new { tableName });
        ...
        /// <param name="tableName"></param>
        /// <returns></returns>
        private string GetCurrentSql(string tableName, string dbService = null)
        {
            string sql;
            if (DBType.Name.ToLower() == DbCurrentType.MySql.ToString().ToLower())
            {
                sql = GetMySqlStructure(tableName, dbService);
            }
        ...
        public async Task<WebResponseContent> SyncTable(string tableName)
        {
          ...
          .Include(o => o.TableColumns).FirstOrDefault();
            ...
            string sql = GetCurrentSql(tableName,tableInfo.DBServer);

            //获取表结构
            List<Sys_TableColumn> columns = GetDapper(tableInfo.DBServer)
        ...
        /// <param name="tableName"></param>
        /// <returns></returns>
        private string GetMySqlStructure(string tableName, string dbService)
        {
            ...
            CASE
                ...
                FROM
                    information_schema.COLUMNS
                WHERE
                    table_name = ?tableName {GetMysqlTableSchema(dbService)}";
        }
        ...
        private int InitTable(int parentId, string tableName, string columnCNName, string nameSpace, string foldername, int tableId, bool isTreeLoad,string dbServer)
        {
            ...
            List<Sys_TableColumn> columns = GetDapper(dbServer)
                .QueryList<Sys_TableColumn>(GetCurrentSql(tableName,dbServer), new { tableName });
        ...
        private string GetConnection(string dbServer)
        {
            ...
            {
                case "ServiceDbContext":
                    sqlDapper = DBServerProvider.ServiceConnectingString;
                    break;
                case "ReportDbContext":
                    sqlDapper = DBServerProvider.ReportConnectingString;
                    break;
                default:
                    sqlDapper = DBServerProvider.SysConnectingString;
                    break;
            }
            return sqlDapper;
        }
        ...
        private string CreateEntityModel(List<Sys_TableColumn> sysColumn, Sys_TableInfo tableInfo, List<TableColumnInfo> tableColumnInfoList, int createType)
        {
            ...
            string baseEntityName = DBServerProvider.GetContextName(tableInfo.DBServer);
