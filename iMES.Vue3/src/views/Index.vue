<template>
  <div id="mes-container" :class="['mes-theme-' + theme]">
    <div class="mes-aside" :style="{ width: menuWidth + 'px' }">
      ...
      <div class="mes-menu">
        <el-scrollbar style="height: 100%">
          <!-- <el-select v-model="value" class="topselect" @change="selectMenu($event)" filterable placeholder="请输入搜索菜单">
            <el-option
          ...
          <el-select v-model="value" class="topselect" @change="selectMenu($event)" filterable  placeholder="请选择">
            <el-option-group
    ...
    <div class="mes-container" :style="{ left: menuWidth - 1 + 'px' }">
      <div class="mes-header">
        <div class="project-name">您的新一代工厂管家【开源版】</div>
        ...
        <div class="header-info">
          <div class="h-link">
            <a href="javascript:void(0)"  @click="drawer_model = true" >
        ...
        <div v-show="contextMenuVisible">
          ...
          >
            <li v-show="visibleItem.all">
              <el-button type="text" @click="closeTabs()">
            ...
            <li v-show="visibleItem.left">
              <el-button type="text" @click="closeTabs('left')" 
                ><i class="el-icon-back"></i>关闭左边</el-button
              >
            </li>
            <li v-show="visibleItem.right">
              <el-button type="text" @click="closeTabs('right')">
...
import MesMenu from "@/components/basic/MesElementMenu.vue";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const changeTheme = (name) => {
      ...
      localStorage.setItem('imes_theme', name);
    };
    const to = (item) => {
     ...
     */
    const created = () => {
      let _theme = localStorage.getItem('imes_theme');
      if (_theme) {
        theme.value = _theme;
      }
  ...
  mounted() {
    ...
    http.get("api/menu/getTreeMenu", {}, true).then((data) => {
        let menuRoot = data.filter(item => item.parentId != 0 && item.url !="/home" )
        this.optionsRoot = menuRoot.map(item => ({ value:item.url,label:item.name}));

        let menu = data.filter(item => item.parentId == 0 && item.url !="/home" )
        let menuData = [];
        menu.forEach(function(ele){
          let menuchild = data.filter(item => item.parentId == ele.id)
          menuchild = menuchild.map(item => ({ value:item.url,label:item.name}));
          ele.options = menuchild;
          ele.label = ele.name;
          menuData.push(ele);
        })
        this.options = menuData;
    });
  ...
  methods: {
    ...
    selectMenu(e){
      let menuName = this.optionsRoot.find(item => item.value==e);
      this.$tabs.open({
                text: menuName.label,
                path: e
              });
...
.mes-container .mes-path ::v-deep(.el-tabs__content) {
  padding: 0;
}
