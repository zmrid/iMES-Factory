...
import FormExpand from './MesForm/MesFormRender';
import {
...
export default defineComponent({
  components: {
    FormExpand,
    'mes-upload': defineAsyncComponent(() =>
      import('@/components/basic/MesUpload.vue')
    ),
    'mes-wang-editor': defineAsyncComponent(() =>
      import('@/components/editor/MesWangEditor.vue')
    )
  },
  props: {
    loadKey: {
  ...
  methods: {
    ...
    fileOnChange(files, item) {
      this.$refs.mesform.clearValidate(item.field);
      if (item.onChange) {
